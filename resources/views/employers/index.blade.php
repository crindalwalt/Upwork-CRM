        ...
        <section class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <form method="GET" action="{{ route('employers.index') }}" class="grid gap-3 xl:grid-cols-[minmax(0,1fr)_160px_170px_180px_auto]">
                <label class="block">
                    ...
                    <input type="text" name="search" value="{{ $filters['search'] }}" placeholder="Company, location, or notes" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-3.5 py-2.5 text-sm text-gray-700 shadow-sm focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-200">
                </label>

                <label class="block">
                    <span class="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-gray-500">Flag</span>
                    <select name="flag" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-3.5 py-2.5 text-sm text-gray-700 shadow-sm focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-200">
                ...
                <label class="block">
                    <span class="text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-gray-500">Verified</span>
                    <select name="verified" class="mt-2 block w-full rounded-xl border border-gray-200 bg-white px-3.5 py-2.5 text-sm text-gray-700 shadow-sm focus:border-violet-400 focus:outline-none focus:ring-2 focus:ring-violet-200">
                        <option value="">Any</option>
                ...
                <div class="flex items-end gap-3">
                    ...
                    <a href="{{ route('employers.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2.5 text-sm font-medium text-white hover:bg-violet-700">
            ...
            @if ($employers->isEmpty())
                <div class="p-6">
                    <x-empty-state icon="ti ti-building-skyscraper" title="No employers match your filters" description="Add a client profile to link jobs, proposals, and trust signals in one place.">
                        <a href="{{ route('employers.create') }}" class="rounded-lg bg-gray-900 px-4 py-2.5 text-sm font-medium text-white hover:bg-black">Add employer</a>
                    </x-empty-state>
                </div>
            @else
                                ...
                                <tr>
                                    <td class="px-6 py-4 align-top">
                                        ...
                                        <div class="mt-2 inline-flex items-center gap-2 text-xs">
                                            <span class="rounded-full border px-2.5 py-1 font-semibold {{ $employer->flagColor() === 'green' ? 'border-gray-900 bg-gray-900 text-white' : ($employer->flagColor() === 'yellow' ? 'border-gray-300 bg-gray-100 text-gray-800' : ($employer->flagColor() === 'red' ? 'border-gray-200 bg-white text-gray-500' : 'border-gray-200 bg-gray-50 text-gray-700')) }}">{{ ucfirst($employer->flagColor()) }}</span>
                                            @if ($employer->upwork_url)
                                                <a href="{{ $employer->upwork_url }}" target="_blank" rel="noreferrer" class="font-semibold text-gray-900 hover:text-black">Upwork</a>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 align-top">
                                        <span class="inline-flex rounded-full border border-gray-300 bg-gray-100 px-3 py-1 text-xs font-semibold text-gray-800">{{ number_format($employer->qualityScore(), 1) }}/10</span>
                                    ...
                                    <td class="px-6 py-4 align-top">
                                        <div class="flex items-center justify-end gap-2 text-gray-500">
                                            <a href="{{ route('employers.show', $employer) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700" title="View">
                                                <i class="ti ti-eye"></i>
                                            </a>
                                            <a href="{{ route('employers.edit', $employer) }}" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700" title="Edit">
                                                <i class="ti ti-edit"></i>
                                            </a>
                                            @can('delete', $employer)
                                                <x-confirm-delete :action="route('employers.destroy', $employer)" title="Delete employer" message="Delete is blocked if the employer is already linked to jobs or proposals." class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 hover:border-red-200 hover:bg-red-50 hover:text-red-700">
